...
trait AutoFindMorphToRelationMethodName
{
        ...
        private function auto_extract_relation_method_name_from_models(Model $currentModel)
        {
            // get class name array
            $model_class_name_array = explode('\\' , get_class($currentModel));

            // get class name
            $model_class_name = $model_class_name_array[count($model_class_name_array) - 1];

            // relation method is strtolower of in model class name
            $relation_method_name = strtolower($model_class_name) . 'able';
